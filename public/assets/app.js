...
const STATE_LABEL = { open: "abierta", partial: "parcial", closed: "cerrada" };
...
async function render() {
  const openTickers = [...new Set(computeFund(doc).open.map((t) => t.ticker))];
  let quotes = {};
...
function paint(f, quotesError) {
  ...
  card("Resultado realizado", signed(f.realized), cls(f.realized),
    `${f.exits.length} ventas ejecutadas`);
...
function contribution(f) {
  ...
  for (const t of rows) {
    ...
    row.innerHTML =
      `<div class="tk">${t.ticker}<i>${STATE_LABEL[t.state]}</i></div>` +
...
function openTable(f, colors) {
  ...
  for (const t of f.open) {
    const badges =
      (t.state === "partial" ? `<i class="badge part">parcial</i>` : "") +
      (t.priceSource === "fallback" ? `<i class="badge">planilla</i>` : "");
    const tr = el("tr", t.belowStop ? "alert" : "");
    tr.innerHTML =
      `<td><i class="chip" style="background:${colors[t.ticker]}"></i>${t.ticker}${badges}</td>` +
      `<td>${qtyFmt.format(t.openQty)}${t.soldQty ? `<i class="of">de ${qtyFmt.format(t.qty)}</i>` : ""}</td>` +
      `<td>${money4.format(t.buyPrice)}</td>` +
      `<td>${money4.format(t.price)}</td>` +
      `<td>${money.format(t.costOpen)}</td>` +
      `<td>${money.format(t.marketValue)}</td>` +
      `<td class="${cls(t.unrealized)}">${signed(t.unrealized)}</td>` +
      `<td class="${cls(t.unrealized)}">${pct(t.costOpen ? t.unrealized / t.costOpen : 0)}</td>` +
  ...
  $("#open-total").innerHTML =
    `<td>Total</td><td>${qtyFmt.format(f.open.reduce((a, t) => a + t.openQty, 0))}</td><td></td><td></td>` +
...
function closedTable(f) {
  ...
  for (const e of f.exits) {
    const tr = el("tr");
    tr.innerHTML =
      `<td>${e.ticker}${e.leg ? `<i class="badge part">venta ${e.leg}</i>` : ""}</td>` +
      `<td class="m">${shortDate(e.date)}</td><td>${e.days}</td>` +
      `<td>${qtyFmt.format(e.qty)}</td><td>${money4.format(e.buyPrice)}</td><td>${money4.format(e.price)}</td>` +
      `<td>${money.format(e.cost)}</td><td>${money.format(e.proceeds)}</td>` +
      `<td class="${cls(e.pnl)}">${signed(e.pnl)}</td><td class="${cls(e.pnl)}">${pct(e.pnlPct)}</td>`;
    body.append(tr);
  }
  const cost = f.exits.reduce((a, e) => a + e.cost, 0);
  const proc = f.exits.reduce((a, e) => a + e.proceeds, 0);
  $("#closed-total").innerHTML =
    `<td>Total</td><td></td><td class="m">${pctFmt.format(f.metrics.avgHoldExits)} prom.</td>` +
    `<td>${qtyFmt.format(f.exits.reduce((a, e) => a + e.qty, 0))}</td><td></td><td></td>` +
  ...
  $("#closed-count").textContent = `${f.exits.length} · ${signed(f.realized)}`;
}
...
function metricsTable(f) {
  ...
  const rows = [
    ...
    ["Aciertos", `${m.winCount} / ${m.tradeCount}`,
      `${pctFmt.format(m.hitRate * 100)}% de las operaciones; ${pctFmt.format(m.exitHitRate * 100)}% de las ${m.exitCount} ventas.`, ""],
    ...
    ["Duración media hasta la venta", `${pctFmt.format(m.avgHoldExits)} días`, "Promedio de las ventas ya ejecutadas.", ""],
